...
import static com.itrip.common.web.Constants.SPT;
...
import com.itrip.cms.entity.assist.CmsComment;
import com.itrip.cms.entity.main.Channel.AfterCheckEnum;
import com.itrip.cms.entity.main.base.BaseContent;
import com.itrip.cms.staticpage.StaticPageUtils;
import com.itrip.cms.web.CmsThreadVariable;
import com.itrip.core.web.ContentInterface;
...
public class Content extends BaseContent implements ContentInterface {
	...
	public String getUrlStatic(Boolean whole, int pageNo) {
		...
		if (!StringUtils.isBlank(filename)) {
			...
			url.append(SPT).append(getChannelRootPath(getChannel()));
			// url.append(df.format(getReleaseDate())); markwy
	 ...
	 * @return
	 */
	private String getChannelRootPath(Channel channel) {
		Channel c = channel;
		Channel level1 = c;
	...
	public String getStaticFilename(int pageNo) {
		...
		if (!StringUtils.isBlank(filename)) {
			...
			// 默认静态路径
			url.append(SPT).append(getChannelRootPath(getChannel()));
			// url.append(df.format(getReleaseDate())); //去掉静态页前面的日期目录
			url.append(SPT).append(getId());
			if (pageNo > 1) {
				url.append("_").append(pageNo);
			}
	...
	public String getStaticFilenameByRule() {
		...
		String url = StaticPageUtils.staticUrlRule(rule, model.getId(), model.getPath(), channel.getId(), channel.getPath(), getId(),
				getReleaseDate());
		return url;
	}
	...
	public String getTxtByNo(int pageNo) {
		ContentTxt txt = getContentTxt();
		if (txt != null) {
			String tmp = txt.getTxtByNo(pageNo);
			/*
			 * 将形如：src="/(p-img||file)/20150110/test.jpg"
			 * 替换为：src="http://picguide.itrip.com/(p-img||file)/20150110/test.jpg"
			 */
			String reg = "src=\"/([\\w-]+)/(.+?)\"";
			tmp = tmp.replaceAll(reg, "src=\"" + Constants.ABSOLUTE_PATH + "/$1/$2\"");
			return tmp;
		} else {
			return null;
		}
	...
	public String getContentDescription() {
		ContentExt ext = getContentExt();
		if (ext != null) {
			return ext.getContentDescription();
		} else {
			return null;
		}
	}
	...
	public String getTitleImgUrl() {
		String url = getTypeImg();
		if (StringUtils.isNotBlank(url)) {
			if (url.startsWith("/p-img") || url.startsWith("/file")) {
	 ...
	 */
	public Content(java.lang.Integer id, com.itrip.cms.entity.main.CmsSite site, java.util.Date sortDate, java.lang.Byte topLevel,
