...
import com.itrip.cms.dao.assist.CmsAcquisitionTempDao;
...
public class CmsAcquisitionTempDaoImpl extends HibernateBaseDao<CmsAcquisitionTemp, Integer> implements CmsAcquisitionTempDao {
	...
	public Integer getPercent(Integer siteId) {
		Query query = getSession().createQuery("select max(percent) from CmsAcquisitionTemp where site.id=:siteId").setParameter("siteId", siteId);
