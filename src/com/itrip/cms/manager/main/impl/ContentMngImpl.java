...
import static com.itrip.cms.entity.main.ContentCheck.DRAFT;
...
import com.itrip.cms.dao.main.ContentDao;
...
import com.itrip.cms.entity.main.ContentTag;
import com.itrip.cms.entity.main.ContentTxt;
import com.itrip.cms.entity.main.Channel.AfterCheckEnum;
import com.itrip.cms.entity.main.Content.ContentStatus;
import com.itrip.cms.manager.assist.CmsCommentMng;
...
import com.itrip.cms.manager.main.ContentCheckMng;
import com.itrip.cms.manager.main.ContentCountMng;
import com.itrip.cms.manager.main.ContentExtMng;
import com.itrip.cms.manager.main.ContentMng;
import com.itrip.cms.manager.main.ContentTagMng;
import com.itrip.cms.manager.main.ContentTxtMng;
import com.itrip.cms.manager.main.ContentTypeMng;
import com.itrip.cms.service.ChannelDeleteChecker;
import com.itrip.cms.service.ContentListener;
import com.itrip.cms.staticpage.StaticPageSvc;
...
public class ContentMngImpl implements ContentMng, ChannelDeleteChecker {
	...
	public Pagination getPageByRight(Integer id, String title, Integer typeId, Integer inputUserId, boolean topLevel, boolean recommend,
		...
		if (allChannel && selfData) {
			// 拥有所有栏目权限，只能管理自己的数据
			p = dao.getPageBySelf(id, title, typeId, inputUserId, topLevel, recommend, status, checkStep, siteId, channelId, userId, needRegenerate,
					orderBy, pageNo, pageSize);
		} else if (allChannel && !selfData) {
			// 拥有所有栏目权限，能够管理不属于自己的数据
			p = dao.getPage(id, title, typeId, inputUserId, topLevel, recommend, status, checkStep, siteId, channelId, needRegenerate, orderBy,
					pageNo, pageSize);
		} else {
			p = dao.getPageByRight(id, title, typeId, inputUserId, topLevel, recommend, status, checkStep, siteId, channelId, userId, selfData,
					needRegenerate, orderBy, pageNo, pageSize);
		}
		return p;
	}
	...
	public Pagination getPageForMember(String title, Integer channelId, Integer siteId, Integer memberId, int pageNo, int pageSize) {
		return dao.getPage(null, title, null, memberId, false, false, ContentStatus.all, null, siteId, channelId, false, 0, pageNo, pageSize);
	}
	...
	private Content saveContent(Content bean, ContentExt ext, ContentTxt txt, Integer channelId, Integer typeId, Boolean draft, CmsUser user,
			boolean forMember) {
	...
	public Content[] contentStatic(Integer[] ids) throws TemplateNotFoundException, TemplateParseException, GeneratedZeroStaticPageException,
		...
		for (int i = 0, len = ids.length; i < len; i++) {
			...
			try {
				...
				// 此处不限制文章是否已生成静态页，重新生成新页面
				content.setNeedRegenerate(true);
