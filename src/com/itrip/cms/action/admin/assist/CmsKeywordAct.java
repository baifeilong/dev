...
import com.itrip.cms.entity.assist.CmsKeyword;
...
public class CmsKeywordAct {
	...
	public String save(CmsKeyword bean, HttpServletRequest request, ModelMap model) {
		...
		cmsLogMng.operating(request, "cmsKeyword.log.save", "id=" + bean.getId() + ";name=" + bean.getName());
	...
	public String delete(Integer[] ids, HttpServletRequest request, ModelMap model) {
		...
		for (CmsKeyword bean : beans) {
			...
			cmsLogMng.operating(request, "cmsKeyword.log.delete", "id=" + bean.getId() + ";name=" + bean.getName());
		}
		return list(request, model);
	}
