package com.itrip.cms.action.admin.assist;
...
import com.itrip.cms.entity.assist.CmsMessage;
import com.itrip.cms.entity.assist.CmsReceiverMessage;
...
import com.itrip.cms.manager.assist.CmsMessageMng;
import com.itrip.cms.manager.assist.CmsReceiverMessageMng;
...
public class CmsMessageAct {
	...
	public String list(Integer pageNo, String title, Date sendBeginTime, Date sendEndTime, Boolean status, Integer box, HttpServletRequest request,
		...
		if (box.equals(0)) {
			// 收件箱
			pagination = receiverMessageMng.getPage(site.getId(), null, user.getId(), title, sendBeginTime, sendEndTime, status, box, false,
					cpn(pageNo), CookieUtils.getPageSize(request));
			returnPage = "message/inbox";
		} else if (box.equals(1)) {
			// 发件箱
			pagination = messageMng.getPage(site.getId(), user.getId(), null, title, sendBeginTime, sendEndTime, status, box, false, cpn(pageNo),
			...
			// 垃圾箱(可能从收件箱或者从发件箱转过来)
			pagination = receiverMessageMng.getPage(site.getId(), user.getId(), user.getId(), title, sendBeginTime, sendEndTime, status, box, false,
					cpn(pageNo), CookieUtils.getPageSize(request));
	...
	public String send(CmsMessage message, String username, Integer groupId, Integer pageNo, String title, Date sendBeginTime, Date sendEndTime,
			Boolean status, Integer box, ModelMap model, HttpServletRequest request, HttpServletResponse response) {
		CmsSite site = CmsUtils.getSite(request);
		CmsUser user = CmsUtils.getUser(request);
		Date now = new Date();
		...
		if (msgReceiverUser != null) {
			messageInfoSet(message, receiverMessage, user, msgReceiverUser, now, site, request);
		}
		// 按会员组推送站内信
					...
					for (int i = 0; i < users.size(); i++) {
						...
						if (msgReceiverUser != null) {
							if (!tempUser.equals(msgReceiverUser)) {
								messageInfoSet(tempMsg, tempReceiverMsg, user, tempUser, now, site, request);
							}
						} else {
							messageInfoSet(tempMsg, tempReceiverMsg, user, tempUser, now, site, request);
						}
		...
		return list(pageNo, title, sendBeginTime, sendEndTime, status, 1, request, response, model);
	}

	private void messageInfoSet(CmsMessage message, CmsReceiverMessage receiverMessage, CmsUser sendUser, CmsUser receiverUser, Date sendTime,
			CmsSite site, HttpServletRequest request) {
		message.setMsgBox(1);
		...
		cmsLogMng.operating(request, "cmsMessage.log.send", "id=" + message.getId() + ";title=" + message.getMsgTitle());
	}

	// 存草稿
	@RequestMapping("/message/v_save.do")
	public String save(CmsMessage message, String username, Integer pageNo, String title, Date sendBeginTime, Date sendEndTime, Boolean status,
			Integer box, ModelMap model, HttpServletRequest request, HttpServletResponse response) {
		CmsSite site = CmsUtils.getSite(request);
		CmsUser user = CmsUtils.getUser(request);
		message.setMsgBox(2);
		...
		// 接收端（有一定冗余）
		receiverMessageMng.save(receiverMessage);
		cmsLogMng.operating(request, "cmsMessage.log.save", "id=" + message.getId() + ";title=" + message.getMsgTitle());
		return list(pageNo, title, sendBeginTime, sendEndTime, status, 2, request, response, model);
	}

	// 发送
	@RequestMapping("/message/v_tosend.do")
	public String message_tosend(Integer id, Integer pageNo, String title, Date sendBeginTime, Date sendEndTime, Boolean status, Integer box,
	...
	public String update(CmsMessage message, Integer pageNo, String title, Date sendBeginTime, Date sendEndTime, Boolean status, Integer box,
			ModelMap model, HttpServletRequest request, HttpServletResponse response) {
		...
		cmsLogMng.operating(request, "cmsMessage.log.update", "id=" + message.getId() + ";title=" + message.getMsgTitle());
		return list(pageNo, title, sendBeginTime, sendEndTime, status, box, request, response, model);
	}
		...
		if (user == null) {
			...
			for (Integer i = 0; i < ids.length; i++) {
				...
				if (message != null && message.getMsgSendUser().equals(user)) {
					...
					cmsLogMng.operating(request, "cmsMessage.log.trash", "id=" + message.getId() + ";title=" + message.getMsgTitle());
				}
				if (receiverMessage != null && receiverMessage.getMsgReceiverUser().equals(user)) {
					receiverMessage.setMsgBox(3);
					receiverMessageMng.update(receiverMessage);
					cmsLogMng.operating(request, "cmsMessage.log.trash", "id=" + receiverMessage.getId() + ";title=" + receiverMessage.getMsgTitle());
				}
		...
		if (user == null) {
			...
			for (Integer i = 0; i < ids.length; i++) {
				...
				if (receiverMessage != null && receiverMessage.getMsgReceiverUser().equals(user)) {
					...
					receiverMessageMng.update(receiverMessage);
					cmsLogMng
							.operating(request, "cmsMessage.log.revert", "id=" + receiverMessage.getId() + ";title=" + receiverMessage.getMsgTitle());
				}
		...
		if (user == null) {
			...
			for (Integer i = 0; i < ids.length; i++) {
				...
				if (receiverMessage != null && receiverMessage.getMsgReceiverUser().equals(user)) {
					...
					cmsLogMng.operating(request, "cmsMessage.log.empty", "id=" + message.getId() + ";title=" + message.getMsgTitle());
				}
	...
	public void findUnreadMessagesByUser(HttpServletRequest request, HttpServletResponse response, ModelMap model) throws JSONException {
		...
		if (user == null) {
			...
			List<CmsReceiverMessage> receiverMessages = receiverMessageMng.getList(site.getId(), null, user.getId(), null, null, null, false, 0,
					false);
			object.put("result", true);
			if (receiverMessages != null && receiverMessages.size() > 0) {
