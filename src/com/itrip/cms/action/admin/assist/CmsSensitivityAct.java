...
import com.itrip.cms.entity.assist.CmsSensitivity;
...
public class CmsSensitivityAct {
	...
	public String save(CmsSensitivity bean, HttpServletRequest request, ModelMap model) {
		...
		cmsLogMng.operating(request, "cmsSensitivity.log.save", "id=" + bean.getId() + ";name=" + bean.getSearch());
	...
	public String delete(Integer[] ids, HttpServletRequest request, ModelMap model) {
		...
		for (CmsSensitivity bean : beans) {
			...
			cmsLogMng.operating(request, "cmsSensitivity.log.delete", "id=" + bean.getId() + ";name=" + bean.getSearch());
		}
		model.addAttribute("message", "global.success");
		return list(request, model);
	}
