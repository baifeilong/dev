...
import com.itrip.common.fck.Command;
import com.itrip.common.fck.ResourceType;
import com.itrip.common.fck.UploadResponse;
import com.itrip.common.fck.Utils;
import com.itrip.common.image.ImageScale;
import com.itrip.common.image.ImageUtils;
import com.itrip.common.upload.FileRepository;
import com.itrip.common.upload.UploadUtils;
...
public class FckAct {
	...
	public void upload(@RequestParam(value = "Command", required = false) String commandStr,
			...
			@RequestParam(value = "mark", required = false) Boolean mark, HttpServletRequest request, HttpServletResponse response) throws Exception {
	...
	private File mark(MultipartFile file, MarkConfig conf) throws Exception {
		...
		if (imgMark) {
			...
			imageScale.imageMark(tempFile, tempFile, conf.getMinWidth(), conf.getMinHeight(), conf.getPos(), conf.getOffsetX(), conf.getOffsetY(),
					markImg);
		} else {
			imageScale.imageMark(tempFile, tempFile, conf.getMinWidth(), conf.getMinHeight(), conf.getPos(), conf.getOffsetX(), conf.getOffsetY(),
					conf.getContent(), Color.decode(conf.getColor()), conf.getSize(), conf.getAlpha());
		}
		return tempFile;
	}
